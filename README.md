...
Reload the studio after adding credentials. A “Connected” indicator means the corresponding variables exist; it is not a successful live-provider smoke test. Modal is the exception currently verified at the infrastructure level: its authenticated API and NVIDIA L4 CUDA runtime passed smoke checks. A real PLY inference is still pending. In particular, Lucy's WebRTC signaling adapter is provisional until tested against an authorized session.
...
- Modal deployment is active and its private GPU probe verified an NVIDIA L4 with CUDA and PyTorch. Point-cloud input is supported by the adapter, but a full SpatialLM inference still needs a reconstructed `.ply` smoke fixture. Walkthrough-to-point-cloud reconstruction, camera alignment, depth/occlusion tracking, and calibrated 3D collision/clearance checks are not implemented. Scan results cannot be applied to arbitrary webcam views without alignment.
