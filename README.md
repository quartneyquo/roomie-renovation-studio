...
Reload the studio after adding credentials. A “Connected” indicator means the corresponding variables exist; it is not a successful live-provider smoke test. Modal is verified at the infrastructure level: its authenticated API, NVIDIA L4 CUDA runtime, SpatialLM runtime, and SLAM3R import all passed smoke checks. A full real-room walkthrough inference is still pending. Lucy uses Decart's official SDK with a short-lived client token minted by Convex; a live media session still requires browser smoke testing.
...
- Text-only arbitrary object generation requires direct Decart Lucy and a live camera. While Lucy connects, the stage keeps showing the unmodified camera rather than compositing the example chair.
